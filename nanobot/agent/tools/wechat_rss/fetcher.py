...
class ArticleFetcher:
    ...
    def fetch_since(
        ...
        Args:
            ...
            batch_size: 每次请求的批次大小

        Returns:
            文章列表（按发布时间倒序）
        """
        self._logger.info(f"获取 {since_timestamp} 之后的文章: fakeid={fakeid}")

        all_articles = []
        begin = 0

        while True:
            batch = self.fetch(fakeid, count=batch_size, begin=begin)
            if not batch:
                break
